...
char *line_in_scale = NULL;
int max_line_len_in_scale = 1024;
...
void output_target(FILE* scaleOutput, double value);
void output(FILE* scaleOutput, int index, double value);
char* readline_in_scale(FILE *input);
int clean_up(FILE *fp_restore, FILE *fp, const char *msg);

int my_svm_scale(const char* tobeScaled, const char* scaled, char* restoreFile = NULL, char* saveFile = NULL)
{
	...
	FILE* scaleOutput = fopen(scaled, "w");
	if (scaleOutput == NULL)
	{
	...
	line_in_scale = (char *)malloc(max_line_len_in_scale * sizeof(char));
...
#define SKIP_ELEMENT\
	...
	while (readline_in_scale(fp) != NULL)
	{
		char *p = line_in_scale;
	...
	while (readline_in_scale(fp) != NULL)
	{
		...
		output_target(scaleOutput, target);
		...
		SKIP_TARGET
			...
			{
				for (i = next_index; i<index; i++)
					output(scaleOutput, i, 0);

				output(scaleOutput, index, value);
...
char* readline_in_scale(FILE *input)
{
	...
	if (fgets(line_in_scale, max_line_len_in_scale, input) == NULL)
		return NULL;

	while (strrchr(line_in_scale, '\n') == NULL)
	{
		max_line_len_in_scale *= 2;
		line_in_scale = (char *)realloc(line_in_scale, max_line_len_in_scale);
		len = (int)strlen(line_in_scale);
		if (fgets(line_in_scale + len, max_line_len_in_scale - len, input) == NULL)
			break;
	}
	return line_in_scale;
}
...
void output(FILE* scaleOutput, int index, double value)
{
	...
	if (value != 0)
	{
		fprintf(scaleOutput, "%d:%g ", index, value);
		new_num_nonzeros++;
	}
}
